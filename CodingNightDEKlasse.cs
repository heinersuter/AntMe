...
namespace AntMe.Player.CodingNightDE
{
    /// <summary>
    ...
    /// </summary>
    [Spieler(
        Volkname = "CodingNightDE",   // Hier kannst du den Namen des Volkes festlegen
    ...
    [Kaste(
        Name = "Killer",                    // Name der Berufsgruppe
    ...
    public class CodingNightDEKlasse : Basisameise
    {
        #region Kasten
        ...
        public override string BestimmeKaste(Dictionary<string, int> anzahl)
        {
            ...
            if (anzahl["Standard"] <= anzahl["Killer"])
            {
                return "Standard";
            }
            return "Killer";
        ...
        public override void Tick()
        {
            if (AktuelleLast > 0)
            {
                SprüheMarkierung((Richtung + 180) % 360, 2);
            }
        }
        ...
        private static Point GetVector(int distanz, int winkel)
        {
            var xZurWanze = distanz * Math.Cos(winkel / 180 * Math.PI);
            var yZurWanze = distanz * Math.Sin(winkel / 180 * Math.PI);
            return new Point((int)xZurWanze, (int)yZurWanze);
        }
